...
void My_UIGraphRefresh()
{
    ...
    UICircleDraw(&UI_Circle_t[0], "sc0", UI_Graph_ADD, 9, UI_Color_Purplish_red, 2, 200, 700, 8);
    
    sprintf(UI_State_sta[4].show_Data, "Sucker");
    UICharDraw(&UI_State_sta[4], "ss4", UI_Graph_ADD, 9, UI_Color_Yellow, 15, 2, 100, 700, "Sucker");
...
{
    for (;;) {
        if (flag_referee_init)My_UIGraphRefresh();
		osDelay(50);
    }
}